...
*    DELETE FROM ztable_irn WHERE billingdocno = '0090000267'.
*    DELETE FROM ztable_irn WHERE billingdocno = '0090000316'.

    SELECT FROM
    ztable_irn AS a
    FIELDS
    *
    where a~billingdocno is not INITIAL
    INTO TABLE @DATA(it).
    ...
    LOOP AT it INTO DATA(wa).
      ...
      WHERE a~BillingDocument = @wa-billingdocno
      INTO @DATA(taxno).

      wa-taxinvoiceno = taxno.
      MODIFY ztable_irn FROM @wa.
