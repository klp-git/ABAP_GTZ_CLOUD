CLASS zcl_foc_exp_drv DEFINITION
 PUBLIC
  FINAL
  CREATE PUBLIC .
...
*                  company_code     TYPE string
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS lc_template_name TYPE string VALUE 'zsd_focexp/zsd_focexp'.
ENDCLASS.



CLASS zcl_foc_exp_drv IMPLEMENTATION.
...
*        IF wa_item-SalesDocumentItemCategory = 'TAN'.
        IF wa_item-SalesDocumentItemCategory = 'CBXN'.
          SELECT SINGLE FROM i_billingdocumentitem AS a
...
*        INTO @DATA(lv_desofgoods) PRIVILEGED ACCESS.
        ...
        DATA(lv_material) = wa_item-product.
        SHIFT lv_material LEFT DELETING LEADING '0'.

        SELECT SINGLE FROM zmaterial_table AS a
        FIELDS
        a~trade_name
        WHERE a~mat = @lv_material
        INTO @DATA(lv_desofgoods) PRIVILEGED ACCESS.

        IF lv_desofgoods IS INITIAL.

          SELECT SINGLE FROM I_ProductText AS a
          FIELDS
          a~ProductName
          WHERE a~Product = @wa_item-Product AND
          a~Language = 'E'
          INTO @lv_desofgoods PRIVILEGED ACCESS.
