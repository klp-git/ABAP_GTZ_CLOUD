CLASS zcl_printform_driver_program DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*    INTERFACES if_oo_adt_classrun .
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS lc_template_name_ss TYPE string VALUE 'zsd_dom_tax_inv/zsd_dom_tax_inv'.
...
CLASS ZCL_PRINTFORM_DRIVER_PROGRAM IMPLEMENTATION.
  ...
  METHOD read_posts .
    ...
    k~taxnumber3 AS taxnumber3_sup
...
*CONCATENATE wa_ship-StreetPrefixName1 wa_ship-StreetPrefixName2 wa_ship-cityname wa_ship-PostalCode wa_ship-DistrictName wa_ship-country   INTO wa_ship-StreetName SEPARATED BY SPACE.
    ...
    SELECT SINGLE FROM i_billingdocument AS a
       ...
       LEFT JOIN i_salesdocument AS c ON b~salesdocument = c~salesdocument
       LEFT JOIN i_salesquotation AS d ON c~referencesddocument = d~salesquotation
    ...
    |<AmountInWords></AmountInWords>| &&                        " in export
    |<BillingDate>{ wa_data-billingdate }</BillingDate>| &&
    |<CreationTime>{ wa_data-creationtime }</CreationTime>| &&  " in export
    ...
    |<ReferenceSDDocument>{ wa_data-ReferenceSDDocument }</ReferenceSDDocument>| &&  " in export
    ...
    |<YY1_PLANT_STATE_CODE_BDH>{ wa_data-state_code2 }</YY1_PLANT_STATE_CODE_BDH>| &&  " in export
    ...
    |<YY1_TermsofDeliveryVF_BDH>{ lv_print-YY1_TermsOfDelivery_SD_SDH }</YY1_TermsofDeliveryVF_BDH>| &&   " in export
    |<YY1_dodatebd_BDH>{ wa_data-YY1_DODate_SDH  }</YY1_dodatebd_BDH>| &&
    |<YY1_TransportGST_bd_BDH>{ wa_data-taxnumber3_sup }</YY1_TransportGST_bd_BDH>| &&
...
*      SHIFT wa_item-Product LEFT DELETING LEADING '0'.
      ...
      |<YY1_EXPTAX_QUANTITY_BDI>{ lv_exp-BillingQuantity }</YY1_EXPTAX_QUANTITY_BDI>| &&  " in export
      |<YY1_NO_PKGS_BDI>{ lv_count }</YY1_NO_PKGS_BDI>| &&  " in export
      ...
      |<YY1_Quant_inKG_of_ltr_BDI>{ quant_in_l }</YY1_Quant_inKG_of_ltr_BDI>| &&  " in export
      ...
      |<YY1_net_weight_bd_BDI>{ net_weight }</YY1_net_weight_bd_BDI>| &&  " in export
      |<YY1_pack_kind_BDI>{ pkg_mat }</YY1_pack_kind_BDI>| &&  " in export
      |<YY1_tare_weight_BDI>{ lv_tare }</YY1_tare_weight_BDI>| && " in export
...
*    CONCATENATE lv_header lv_item  INTO lv_header .
    ...
    CONCATENATE lv_header lv_footer0 INTO lv_header.
    ...
    IF printname = 'expo'.
      lv_footer1 =
        |<SoldToParty>| &&
      |<AddressLine1Text>{ wa_sold-CustomerName }</AddressLine1Text>| &&     " pending
      |<AddressLine2Text>{ wa_sold-StreetName }</AddressLine2Text>| &&
      |<AddressLine3Text>{ wa_sold-StreetPrefixName1 }</AddressLine3Text>| &&
      |<AddressLine4Text>{ wa_sold-StreetPrefixName2 }</AddressLine4Text>| &&
      |<AddressLine5Text>{ wa_sold-CityName }</AddressLine5Text>| &&
    ...
    CONCATENATE lv_header lv_footer1 INTO lv_header.
    ...
    CONCATENATE lv_header lv_footer2 INTO lv_header.
    ...
    CONCATENATE lv_header lv_footer3 INTO lv_header.

    DATA(lv_footer4) =
    |</BillingDocumentNode>| &&
    |</Form>|.

    CONCATENATE lv_header lv_footer4 INTO lv_header.
    ...
    IF printname = 'dom'.    " done
      lc_template_name = lc_template_name_dom.
    ENDIF.
    IF printname = 'ss'.     " done
      lc_template_name = lc_template_name_ss.
    ENDIF.
    IF printname = 'foc'.    " done with some mismatched
