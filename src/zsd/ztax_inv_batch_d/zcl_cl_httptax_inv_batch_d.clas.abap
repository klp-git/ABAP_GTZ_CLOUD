...
CLASS ZCL_CL_HTTPTAX_INV_BATCH_D DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_CL_HTTPTAX_INV_BATCH_D IMPLEMENTATION.
...
*     req_uri = request->get_request_uri( ).
    ...
    req_uri = '/sap/bc/http/sap/ZCL_HTTPTAX_INV_BATCH_D?sap-client=080'.
    ...
    CASE request->get_method( ).
      ...
      WHEN CONV string( if_web_http_client=>post ).
        ...
        data: lv_doc type I_BillingDocument-BillingDocument.
        lv_doc = request->get_form_field( `doc` ).
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
        lv_doc =   |{ lv_doc ALPHA = IN }| .
        SELECT SINGLE FROM  I_BillingDocument WITH PRIVILEGED ACCESS
        FIELDS billingdocument WHERE billingdocument = @lv_doc "6000000002
        INTO @DATA(lv_doc2).

        IF lv_doc2 IS NOT INITIAL.

          TRY.
              DATA(pdf) = zcl_tax_inv_batch_d=>read_posts( cleardoc = lv_doc ).
