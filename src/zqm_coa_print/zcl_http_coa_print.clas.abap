CLASS zcl_http_COA_PRINT DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_HTTP_COA_PRINT IMPLEMENTATION.
...
*     req_uri = request->get_request_uri( ).
    ...
    req_uri = '/sap/bc/http/sap/ZHTTP_COA_PRINT?sap-client=080'.
    ...
    CASE request->get_method( ).
      ...
      WHEN CONV string( if_web_http_client=>post ).
        ...
        DATA(lot) = request->get_form_field( `lot` ).

        SELECT SINGLE FROM I_INSPECTIONLot WITH PRIVILEGED ACCESS
        FIELDS InspectionLot WHERE InspectionLot = @lot
        INTO @DATA(lv_lot).

        IF lv_lot IS NOT INITIAL.

          TRY.
              DATA(pdf) = zcl_coa_print=>read_posts( lot = lot ).
    if  pdf = 'ERROR'.
