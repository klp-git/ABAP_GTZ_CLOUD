CLASS zcl_maintaince_order_drv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*                  company_code     TYPE string
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS lc_template_name TYPE string VALUE 'zmm_maintenance_po/zmm_maintenance_po'."'zpo/zpo_v2'."
ENDCLASS.



CLASS ZCL_MAINTAINCE_ORDER_DRV IMPLEMENTATION.
  ...
  METHOD read_posts.
    ...
    IF lv_address-StreetName IS NOT INITIAL.
      IF lv_supp_add IS NOT INITIAL.
        CONCATENATE lv_supp_add ', ' lv_address-StreetName INTO lv_supp_add.
      ELSE.
    ...
    IF lv_address-StreetPrefixName1 IS NOT INITIAL.
      IF lv_supp_add IS NOT INITIAL.
        CONCATENATE lv_supp_add ', ' lv_address-StreetPrefixName1 INTO lv_supp_add.
      ELSE.
    ...
    IF lv_address-StreetPrefixName2 IS NOT INITIAL.
      IF lv_supp_add IS NOT INITIAL.
        CONCATENATE lv_supp_add ', ' lv_address-StreetPrefixName2 INTO lv_supp_add.
      ELSE.
    ...
    IF lv_address-CityName IS NOT INITIAL.
      IF lv_supp_add IS NOT INITIAL.
        CONCATENATE lv_supp_add ', ' lv_address-CityName INTO lv_supp_add.
      ELSE.
    ...
    IF lv_address-PostalCode IS NOT INITIAL.
      IF lv_supp_add IS NOT INITIAL.
        CONCATENATE lv_supp_add ', ' lv_address-PostalCode INTO lv_supp_add.
      ELSE.
    ...
    IF lv_address-DistrictName IS NOT INITIAL.
      IF lv_supp_add IS NOT INITIAL.
        CONCATENATE lv_supp_add ', ' lv_address-DistrictName INTO lv_supp_add.
      ELSE.
    ...
    IF lv_address-RegionName IS NOT INITIAL.
      IF lv_supp_add IS NOT INITIAL.
        CONCATENATE lv_supp_add ', ' lv_address-RegionName INTO lv_supp_add.
      ELSE.
    ...
    IF lv_address-country IS NOT INITIAL.
      IF lv_supp_add IS NOT INITIAL.
        CONCATENATE lv_supp_add ', ' 'India' INTO lv_supp_add.
      ELSE.
        lv_supp_add = 'India'.
      ENDIF.
    ENDIF.
   ...
   INTO @DATA(lv_paymentTerms).
    ...
    SELECT SINGLE
    FROM i_purchaseordernotetp_2 AS a
    FIELDS a~PlainLongText
    WHERE a~PurchaseOrder = @lv_po2 AND a~TextObjectType = 'F01'
    INTO @DATA(insurance_txt).

    SELECT SINGLE
    FROM i_purchaseordernotetp_2 AS a
    FIELDS a~PlainLongText
    WHERE a~PurchaseOrder = @lv_po2 AND a~TextObjectType = 'F07'
    INTO @DATA(PAYMENT_txt).

    SELECT SINGLE
    FROM i_purchaseordernotetp_2 AS a
    FIELDS a~PlainLongText
    WHERE a~PurchaseOrder = @lv_po2 AND a~TextObjectType = 'F08'
    INTO @DATA(WARRANTIES_txt).
...
*    |<insurance>{ lv_insurance }</insurance>| &&      " done
    |<delivery>{ lv_paymentTerms-YY1_DeliveryText_PDH }</delivery>| &&       " done
    |<paymentTerms>{ lv_paymentTerms-YY1_Remarks_PDH }</paymentTerms>| &&       " done
    |<assestscode>{ lv_assets }</assestscode>| &&
    |<headertext>{ lv_headerText }</headertext>| &&
    |<deliveryAdd>{ lv_plant_add }</deliveryAdd>| &&
    |<itemtext>{ lv_paymentterms-YY1_ItemText_PDH }</itemtext>| &&
    |<WARRANTIES>{ WARRANTIES_txt }</WARRANTIES>| &&
    |<PAYMENTTERMSTEXT>{ PAYMENT_txt }</PAYMENTTERMSTEXT>| &&
    |<insurance>{ insurance_txt }</insurance>| &&
    |</HEADER>| &&
    |<Items>|.
...
*      ENDIF.
    ...
    b~gstin_no
    WHERE a~purchaseorder = @lv_po2
    INTO @DATA(lv_gstin) PRIVILEGED ACCESS.
    ...
    |<total_freight_chrg>{ total_frt_chrg }</total_freight_chrg>| &&
