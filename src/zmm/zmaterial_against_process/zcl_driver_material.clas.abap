CLASS zcl_driver_material DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*    INTERFACES if_oo_adt_classrun .
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS lc_template_name TYPE string VALUE 'zmaterial_consumption/zmaterial_consumption'.

ENDCLASS.



CLASS ZCL_DRIVER_MATERIAL IMPLEMENTATION.
...
*   LEFT JOIN I_ProcessOrderComponentTP AS c ON a~Material = c~Material AND a~Material IS NOT INITIAL AND c~Material IS NOT INITIAL and a~batch = c~batch
   FIELDS
...
*   AND c~ProcessOrder = @process_no_1
*   and c~WithdrawnQuantity <> 0
   INTO TABLE @DATA(item).
    ...
    LOOP AT item INTO DATA(wa_item).
      ...
      SELECT SINGLE FROM
      I_ProcessOrderComponentTP AS a
      FIELDS
      SUM( a~WithdrawnQuantity )
      WHERE a~ProcessOrder = @process_no
      AND a~WithdrawnQuantity <> 0
      AND a~material IS NOT INITIAL
      AND a~Material = @wa_item-Material
      AND a~Batch = @wa_item-batch
      INTO @DATA(lv_withdrawn).
      ...
      DATA(lv_item) = |<lineitem>| &&
              ...
              |<Consumption_Qty>{ lv_withdrawn }</Consumption_Qty>| &&
              |</lineitem>|.
...
*        FIELDS
*        c~WithdrawnQuantity,c~Batch
*        WHERE c~ProcessOrder = @process_no_1 AND c~material = @wa_table-Material AND c~RequiredQuantity IS NOT INITIAL
...
*        |<UOM>{ lv_unit }</UOM>| &&
*        |<Batch_No_item>{ wa_required-Batch }</Batch_No_item>| &&
*        |<Expiry_date>{ wa_item1-ShelfLifeExpirationDate }</Expiry_date>| &&
*        |<Issue_qty>{ wa_table-issuedqty }</Issue_qty>| &&
*        |<Consumption_Qty>{ wa_required-WithdrawnQuantity }</Consumption_Qty>| &&
*        |</lineitem>|.
*          CONCATENATE lv_xml lv_item INTO lv_xml.
...
*        WHERE c~ProcessOrder = @process_no_1 AND c~material = @wa_item1-Material AND c~RequiredQuantity IS NOT INITIAL
...
*            |<Expiry_date>{ wa_item1-ShelfLifeExpirationDate }</Expiry_date>| &&
*            |<Issue_qty>{ wa_table-issuedqty }</Issue_qty>| &&
*            |<Consumption_Qty>{ sumrequired }</Consumption_Qty>| &&
*            |</lineitem>|.
*          CONCATENATE lv_xml lv_item INTO lv_xml.
*          CLEAR sumrequired.
*          CLEAR lv_unit.
